# PairBuildr Session Runtime

PairBuildr sessions preserve durable conversational history while assembling the runtime context an agent needs to act correctly in its current environment.
...
A temporary file created under PairBuildr's shared tool-output directory to retain complete output that was too large for Session history.
...
- The **PTY Environment** is a server concern rather than a Core PTY concern. PTY creation merges caller values, then the host overlay, then Core-forced terminal invariants such as `TERM` and `PAIRBUILDR_TERMINAL`.
...
- `sessions.create(...)` accepts an optional `location`. Omission resolves through the connected PairBuildr instance's default or current location; an explicit value selects a known location. Networked and embedded transports use the same handler semantics.
...
- Ambient project instruction discovery honors `PAIRBUILDR_DISABLE_PROJECT_CONFIG`; global instructions remain eligible.
