...
> **Historical.** This spec describes work that has already landed in
> `packages/tui`. It repeatedly references `packages/cli`, an experimental
> second CLI that this fork removed; treat those passages as a record of the
> original plan, not as instructions.

## Goal

Move the canonical PairBuildr terminal application from
`packages/pairbuildr/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/pairbuildr`,
`packages/cli`, or `@pairbuildr/core`.

The SDK is the TUI's PairBuildr boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/pairbuildr`.
- Do not replace private `packages/pairbuildr` imports with `@pairbuildr/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@pairbuildr/tui`.
...
- PairBuildr domain data displayed by the TUI
...
```text
packages/pairbuildr/src/cli/cmd/tui
```

Its private dependency on `packages/pairbuildr` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/pairbuildr/src/*`.
...
Status: Completed. The private `@pairbuildr/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@pairbuildr/tui`.
...
- The package has no dependency on `pairbuildr`, `@pairbuildr/cli`, or
  `@pairbuildr/core`.
...
primitives now live in `@pairbuildr/tui`. The legacy host consumes explicit
...
and build inputs through `@pairbuildr/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads PairBuildr globals or process state;
...
Replace process-global PairBuildr state with resolved TUI inputs.
...
`@pairbuildr/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/pairbuildr` initially.
...
canonical in `@pairbuildr/tui`. Configured references resolve through the new
...
adapters rather than PairBuildr domain implementations.

Make the SDK the only PairBuildr domain boundary used by the TUI.
...
- Domain-facing TUI code imports PairBuildr data and operations only from
  `@pairbuildr/sdk`.
...
`@pairbuildr/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/pairbuildr`.
...
- The package has no imports from `packages/pairbuildr`, `packages/cli`, or
  `@pairbuildr/core`.
...
public `@pairbuildr/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/pairbuildr`.
- Keep the legacy embedded worker and server startup in `packages/pairbuildr`.
- Change those adapters to load config, create transport inputs, and call the
  public `@pairbuildr/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/pairbuildr`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@pairbuildr/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@pairbuildr/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for PairBuildr domain state.
...
cd packages/pairbuildr && bun typecheck
...
rg '@pairbuildr/core|packages/pairbuildr|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/pairbuildr/src packages/pairbuildr/test
```
...
- Build the current-platform `packages/pairbuildr` binary.
