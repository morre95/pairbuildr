...
Both `action` and `resource` use pairbuildr's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.pairbuildr` files is intentionally deferred until `.pairbuildr` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether pairbuildr permits an operation through its managed execution paths.
