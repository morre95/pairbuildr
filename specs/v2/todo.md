...
The pairbuildr server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@pairbuildr/llm` tool loop after replacing its
...
- plugins get global "pairbuildr" instance like in that post i showed
- pairbuildr instance has stuff like `pairbuildr.session.prompt()` or
  `pairbuildr.tool.register({...})`
...
- decide whether to preserve deprecated `@pairbuildr/llm` orchestration exports
