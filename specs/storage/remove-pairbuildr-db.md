# Remove `packages/pairbuildr/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/pairbuildr/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the pairbuildr legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/pairbuildr/src/storage/db.ts` are concentrated in 22 source files:

- `packages/pairbuildr/src/account/repo.ts`
- `packages/pairbuildr/src/cli/cmd/db.ts`
- `packages/pairbuildr/src/cli/cmd/import.ts`
- `packages/pairbuildr/src/cli/cmd/stats.ts`
- `packages/pairbuildr/src/control-plane/workspace.ts`
- `packages/pairbuildr/src/index.ts`
- `packages/pairbuildr/src/node.ts`
- `packages/pairbuildr/src/permission/index.ts`
- `packages/pairbuildr/src/project/project.ts`
- `packages/pairbuildr/src/server/projectors.ts`
- `packages/pairbuildr/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/pairbuildr/src/server/shared/fence.ts`
- `packages/pairbuildr/src/session/message-v2.ts`
- `packages/pairbuildr/src/session/projectors.ts`
- `packages/pairbuildr/src/session/prompt.ts`
- `packages/pairbuildr/src/session/session.ts`
- `packages/pairbuildr/src/session/todo.ts`
- `packages/pairbuildr/src/share/share-next.ts`
- `packages/pairbuildr/src/storage/db.ts`
- `packages/pairbuildr/src/sync/index.ts`
- `packages/pairbuildr/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy pairbuildr database wrapper; `packages/pairbuildr/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the pairbuildr wrapper.
...
Status: Completed. `SyncEvent` and the pairbuildr projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy pairbuildr database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy pairbuildr database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy pairbuildr database wrapper.
...
All migration groups are complete or superseded. `packages/pairbuildr/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No pairbuildr data-migration group remains.

The previous pairbuildr `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate pairbuildr data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/pairbuildr`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/pairbuildr/src`
- `bun typecheck` from `packages/pairbuildr`
- Relevant package tests from `packages/pairbuildr`, not the repo root
