...
const bot = ["actions-user", "github-actions[bot]", "pairbuildr", "pairbuildr-agent[bot]"]
const team = [
  // Optional maintainer roster; absent in a fork without one.
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/pairbuildr packages/tui packages/core packages/sdk packages/plugin`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/pairbuildr/src/cli/cmd/") || file.startsWith("packages/tui/")) areas.add("tui")
      else if (file.startsWith("packages/pairbuildr/") || file.startsWith("packages/core/")) areas.add("core")
