...
Practical reference for a future `packages/server` split after the pairbuildr
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/pairbuildr`.
...
- `packages/pairbuildr` - CLI entrypoint; `packages/tui` - TUI
...
Until enough shared service code lives outside `packages/pairbuildr`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/pairbuildr`

It should not import `packages/pairbuildr` services while `packages/pairbuildr`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on pairbuildr-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/pairbuildr` implementation details.
