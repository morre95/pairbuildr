...
- Author package entrypoint is `@pairbuildr/plugin/tui`.
...
```json
{
  "$schema": "https://pairbuildr.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/pairbuildr-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@pairbuildr/plugin/tui`.
- `@pairbuildr/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@pairbuildr/plugin/tui"
...
- The value is a semver range checked against the running PairBuildr version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.pairbuildr` is absent, no check is performed (backward compatible).
...
- `pairbuildr plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `pairbuildr plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.pairbuildr` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.pairbuildr`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.pairbuildr`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.pairbuildr` dir or `PAIRBUILDR_CONFIG_DIR`, PairBuildr installs `@pairbuildr/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@pairbuildr/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus PairBuildr's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
PairBuildr registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, PairBuildr pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@pairbuildr/plugin/tui`.
...
- `message` is required; `title` defaults to `"pairbuildr"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `pairbuildr.default`.
...
- Local plugins persist installed themes under the local `.pairbuildr/themes` area near the plugin config source.
...
- `--pure` / `PAIRBUILDR_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.pairbuildr/plugins/tui-smoke.tsx`
- Local vim plugin: `.pairbuildr/plugins/tui-vim.tsx`
- Local smoke config: `.pairbuildr/tui.json`
- Local smoke theme: `.pairbuildr/plugins/smoke-theme.json`
