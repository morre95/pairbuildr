...
Delete `packages/pairbuildr/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/pairbuildr/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/pairbuildr/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/pairbuildr`
- TUI plugin loader tests in `packages/pairbuildr` if runtime plugin API wiring changed
