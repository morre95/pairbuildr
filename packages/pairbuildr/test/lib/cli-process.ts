// Subprocess test harness for the pairbuildr CLI. Spawns the real binary against
...
//
// Configuration flows through pairbuildr's built-in test affordances:
//   - PAIRBUILDR_CONFIG_CONTENT      : provider config inline, no files to find
//   - PAIRBUILDR_TEST_HOME           : pins os.homedir() → tmpdir
//   - PAIRBUILDR_DISABLE_PROJECT_CONFIG : skip walking up for pairbuildr.json
//   - PAIRBUILDR_PURE                : skip external plugin discovery + install
//   - PAIRBUILDR_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `pairbuildr.run` is fully wired. The shape supports adding more
// builders (`pairbuildr.serve(opts)`, `pairbuildr.acp(opts)`, `pairbuildr.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of PairbuildrCli.
...
import { AppProcess } from "@pairbuildr/core/process"
...
const pairbuildrRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(pairbuildrRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    PAIRBUILDR_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    PAIRBUILDR_CONFIG_CONTENT: configJson,
    PAIRBUILDR_DISABLE_PROJECT_CONFIG: "1",
    PAIRBUILDR_PURE: "1",
    PAIRBUILDR_DISABLE_AUTOUPDATE: "1",
    PAIRBUILDR_DISABLE_AUTOCOMPACT: "1",
    PAIRBUILDR_DISABLE_MODELS_FETCH: "1",
    PAIRBUILDR_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `pairbuildr run`. New flags should
...
// `pairbuildr serve` is a long-lived process — it never exits on its own.
...
// `pairbuildr acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type PairbuildrCli = {
  ...
  // Spawn `pairbuildr serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `pairbuildr acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly pairbuildr: PairbuildrCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("pairbuildr.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          PAIRBUILDR_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("pairbuildr.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("pairbuildr.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "pairbuildr server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `pairbuildr serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("pairbuildr.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const pairbuildr: PairbuildrCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, pairbuildr })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `pairbuildr.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "pairbuildr") {
...
// every call site. Use this for any test that needs the pairbuildr CLI fixture.
//
...
// resources (e.g. `pairbuildr.serve`) without an extra `Effect.scoped` wrapper —
