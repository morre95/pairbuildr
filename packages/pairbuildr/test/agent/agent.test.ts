...
it.instance("plan agent denies edits except .pairbuildr/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".pairbuildr/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".pairbuildr", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.PAIRBUILDR_TEST_HOME
      process.env.PAIRBUILDR_TEST_HOME = test.directory
