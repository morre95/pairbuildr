...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "pairbuildr.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "pairbuildr.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "pairbuildr.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "pairbuildr.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["pairbuildr.json", "pairbuildr.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "pairbuildr.json"),
        path.join(project, "pairbuildr.jsonc"),
      ])
    })
  })
