...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.PAIRBUILDR_TEST ?? "")'), {
      env: {
        PAIRBUILDR_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "PAIRBUILDR_TEST_SHELL"], {
      shell: true,
      env: {
        PAIRBUILDR_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("PAIRBUILDR_TEST_SHELL=ok")
  })
