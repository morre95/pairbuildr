...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.PAIRBUILDR_WEBSEARCH_PROVIDER

    try {
      process.env.PAIRBUILDR_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.PAIRBUILDR_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.PAIRBUILDR_WEBSEARCH_PROVIDER
      else process.env.PAIRBUILDR_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled by an explicit websearch provider flag", () => {
    // Upstream also enabled it implicitly for its hosted provider; that is removed,
    // so a search backend must be configured explicitly regardless of provider.
    expect(webSearchEnabled(ProviderV2.ID.openai, { exa: false, parallel: false })).toBe(false)
    expect(webSearchEnabled(ProviderV2.ID.anthropic, { exa: false, parallel: false })).toBe(false)
