...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".pairbuildr")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .pairbuildr/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const pairbuildr = path.join(test.directory, ".pairbuildr")
      const tool = path.join(pairbuildr, "tool")
  ...
  it.instance("ignores non-tool exports in .pairbuildr/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".pairbuildr", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-pairbuildr` was registering a tool
  ...
  it.instance("loads tools from .pairbuildr/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const pairbuildr = path.join(test.directory, ".pairbuildr")
      const tools = path.join(pairbuildr, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".pairbuildr", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const pairbuildr = path.join(test.directory, ".pairbuildr")
        const customTools = path.join(pairbuildr, "tools")
        const plugin = path.join(pairbuildr, "node_modules", "@pairbuildr", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(pairbuildr, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@pairbuildr/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@pairbuildr/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".pairbuildr", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(pairbuildr, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@pairbuildr/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(pairbuildr, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(pairbuildr, "node_modules", "cowsay")
