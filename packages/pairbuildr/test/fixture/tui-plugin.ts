...
import type { HostPluginApi } from "@pairbuildr/tui/plugin/slots"
...
export function createTuiPluginApi(opts: Opts = {}): HostPluginApi {
  ...
  const own = createPairbuildrClient({
  ...
  let selected = opts.theme?.selected ?? "blueprint"
  const set =
    opts.theme?.set ??
    ((name: string) => {
  ...
  return {
    ...
    attention: {
      ...
      soundboard: {
        ...
        current: () => opts.attention?.soundboard?.current?.() ?? "pairbuildr.default",
