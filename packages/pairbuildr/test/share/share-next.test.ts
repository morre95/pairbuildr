...
describe("ShareNext", () => {
  ...
  it.live("request fails when no share host is configured", () =>
    provideTmpdirInstance(() =>
      ShareNext.Service.use((svc) =>
        Effect.gen(function* () {
          // Sharing has no default host. Upstream defaulted to its own hosted service;
          // this fork refuses rather than posting a session somewhere the user did not choose.
