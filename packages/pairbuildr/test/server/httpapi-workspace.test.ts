...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-pairbuildr-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-pairbuildr-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://pairbuildr.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-pairbuildr-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-pairbuildr-workspace")
