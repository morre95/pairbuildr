...
const stateLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = {
      PAIRBUILDR_EXPERIMENTAL_WORKSPACES: Flag.PAIRBUILDR_EXPERIMENTAL_WORKSPACES,
    }
    ...
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.PAIRBUILDR_EXPERIMENTAL_WORKSPACES = original.PAIRBUILDR_EXPERIMENTAL_WORKSPACES
        await resetDatabase()
      }),
    )
  }),
