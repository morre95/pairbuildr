...
import type { Event, PairbuildrClient } from "@pairbuildr/sdk/v2"
...
type PermissionReplyParams = Parameters<PairbuildrClient["permission"]["reply"]>[0]
...
function createHarness(
  ...
  } as unknown as PairbuildrClient
  const connection = {
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "pairbuildr-acp-permission-"))
