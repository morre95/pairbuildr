...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ pairbuildr }) =>
    Effect.gen(function* () {
      const result = yield* pairbuildr.spawn(["--replay-limit", "10"])

      pairbuildr.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ pairbuildr }) =>
    Effect.gen(function* () {
      const result = yield* pairbuildr.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ pairbuildr }) =>
    Effect.gen(function* () {
      const result = yield* pairbuildr.spawn(["--mini", "--port", "4096"])
