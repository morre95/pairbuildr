// Subprocess integration tests for `pairbuildr run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `pairbuildr.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `PAIRBUILDR_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("pairbuildr run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pairbuildr.run("say hi")
        pairbuildr.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pairbuildr.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* pairbuildr.run("think", { extraArgs: ["--thinking"] })
        pairbuildr.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* pairbuildr.run("think again")
        pairbuildr.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pairbuildr.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pairbuildr.run("say hi", { format: "json" })
        pairbuildr.expectExit(result, 0)

        const events = pairbuildr.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const result = yield* pairbuildr.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pairbuildr.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const result = yield* pairbuildr.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* pairbuildr.run("request permission", { permission: { bash: "ask" } })
        pairbuildr.expectExit(denied, 0)
        ...
        const allowed = yield* pairbuildr.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        pairbuildr.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* pairbuildr.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        pairbuildr.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, pairbuildr }) =>
      Effect.gen(function* () {
        ...
        const server = yield* pairbuildr.serve()

        const result = yield* pairbuildr.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, pairbuildr }) =>
      Effect.gen(function* () {
        const result = yield* pairbuildr.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, pairbuildr }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* pairbuildr.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
