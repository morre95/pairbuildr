...
import { PairbuildrClient, type GlobalEvent } from "@pairbuildr/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<PairbuildrClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<PairbuildrClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<PairbuildrClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<PairbuildrClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<PairbuildrClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://pairbuildr.test"),
...
function sdk(
  input: {
    ...
    subscribe?: PairbuildrClient["event"]["subscribe"]
    globalEvent?: PairbuildrClient["global"]["event"]
    promptAsync?: PairbuildrClient["session"]["promptAsync"]
    status?: PairbuildrClient["session"]["status"]
    messages?: PairbuildrClient["session"]["messages"]
    children?: PairbuildrClient["session"]["children"]
    permissions?: PairbuildrClient["permission"]["list"]
    questions?: PairbuildrClient["question"]["list"]
  } = {},
) {
  const client = new PairbuildrClient()

  const subscribe: PairbuildrClient["event"]["subscribe"] =
    input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: PairbuildrClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: PairbuildrClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: PairbuildrClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: PairbuildrClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: PairbuildrClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: PairbuildrClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: PairbuildrClient["question"]["list"] = input.questions ?? (() => ok([]))
