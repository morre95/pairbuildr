...
describe("pairbuildr read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // PAIRBUILDR_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const r = yield* pairbuildr.spawn(["mcp", "list"])
        pairbuildr.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const r = yield* pairbuildr.spawn(["providers", "list"])
        pairbuildr.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const r = yield* pairbuildr.spawn(["models"])
        pairbuildr.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const r = yield* pairbuildr.spawn(["agent", "list"])
        pairbuildr.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh PAIRBUILDR_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const r = yield* pairbuildr.spawn(["session", "list"])
        pairbuildr.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const r = yield* pairbuildr.spawn(["stats"])
        pairbuildr.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under PAIRBUILDR_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const r = yield* pairbuildr.spawn(["db", "path"])
        pairbuildr.expectExit(r, 0, "db path")
