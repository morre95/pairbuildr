...
describe("pairbuildr acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ pairbuildr }))
        ...
        expect(initialized.agentInfo?.name).toBe("PairBuildr")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ pairbuildr })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("pairbuildr-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(
          yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "pairbuildr-login" }),
        ).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.PAIRBUILDR_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ pairbuildr }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("pairbuildr-login")
