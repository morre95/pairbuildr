...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/pairbuildr.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/pairbuildr.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/pairbuildr.jsonc", dir: ".pairbuildr", suggestion: "pairbuildr" },
        expected:
          'Directory ".pairbuildr" in /tmp/pairbuildr.jsonc is not valid. Rename the directory to "pairbuildr" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/pairbuildr.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/pairbuildr.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    ...
    expect(formatted).toContain("Could not reach POST https://auth.example.com/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `pairbuildr models` to list available models",
      "Or check your config (pairbuildr.json) provider/model names",
    ].join("\n")
