...
import { LLMEvent, ToolFailure } from "@pairbuildr/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@pairbuildr/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("openai-compatible"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    // Upstream additionally accepted its hosted "opencode*" provider IDs here. Those
    // are removed, so an arbitrary OpenAI-compatible provider is no longer native.
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("openai-compatible") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("openai-compatible") },
        auth: undefined,
      }),
