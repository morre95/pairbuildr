...
import { SessionMessageTable } from "@pairbuildr/core/session/sql"
...
import { Shell } from "@pairbuildr/core/shell"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "pairbuildr.json"),
    JSON.stringify({ $schema: "https://pairbuildr.ai/config.json", ...config }),
  )
})
...
noLLMServer.instance("prompt tools replace previous prompt tool rules", () =>
  ...
  // Needs an explicit provider: upstream fell back to its hosted provider, which is gone.
  { config: cfg },
)
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/pairbuildr/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("openai"), modelID: ModelV2.ID.make("gpt-5-nano") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
