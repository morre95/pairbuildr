...
import { LLMEvent, LLMResponse } from "@pairbuildr/llm"
import { RequestExecutor } from "@pairbuildr/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.PAIRBUILDR_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["pairbuildr", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("PAIRBUILDR_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("PAIRBUILDR_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["pairbuildr", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    tags: ["pairbuildr", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("PAIRBUILDR_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("PAIRBUILDR_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("PAIRBUILDR_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set PAIRBUILDR_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the pairbuildr LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "pairbuildr.json"),
      JSON.stringify({ $schema: "https://pairbuildr.ai/config.json", ...scenario.config(model) }),
    ),
  )
