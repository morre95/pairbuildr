      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "reasoning", text: text("assistant reasoning") },
          {
            ...
            toolCallId: "call-1",
            toolName: "Read",
            input: { filePath: ".pairbuildr/tool/emoji.ts" },
          },
          {
            type: "tool-result",
            toolCallId: "call-2",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const pairbuildrModel = {
      ...openaiModel,
      providerID: "pairbuildr",
      api: {
        id: "pairbuildr-test",
        url: "https://api.pairbuildr.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, pairbuildrModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.pairbuildr?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.pairbuildr?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          pairbuildr: { itemId: "msg_pairbuildr" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              pairbuildr: { itemId: "msg_pairbuildr_part" },
    ...
    expect(result[0].providerOptions?.pairbuildr?.itemId).toBe("msg_pairbuildr")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.pairbuildr?.itemId).toBe("msg_pairbuildr_part")
