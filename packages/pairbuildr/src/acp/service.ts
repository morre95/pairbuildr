...
import type { AssistantMessage, Message, PairbuildrClient, SessionMessageResponse } from "@pairbuildr/sdk/v2"
...
export const AuthMethodID = "pairbuildr-login"
...
export class Service extends Context.Service<Service, Interface>()("@pairbuildr/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `pairbuildr auth login` in the terminal",
      name: "Login with pairbuildr",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: PairbuildrClient) {
...
function makeUsageService(sdk: PairbuildrClient) {
...
async function loadDirectorySnapshot(sdk: PairbuildrClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // First-session ACP startup must not scan historical sessions just to infer a
  // default. Upstream preferred its own hosted provider here; with that removed the
  // configured model, then the sorted best model, keep the response deterministic
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "PairBuildr prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "PairBuildr service failure", service })
}
