import type { NamedError } from "@pairbuildr/core/util/error"
...
// `action` stays part of the shape even though no branch sets one today: it is an
// optional field of the public session-status contract, and the only producers were
// upstream's hosted-subscription upsells, which this fork removed.
type Retry = {
  message: string
  action?: {
    reason: string
...
export function retryable(error: Err, provider: string): Retry | undefined {
