...
import type { LLMClientShape } from "@pairbuildr/llm/route"
...
function statusWithFetch(
  ...
  // Upstream also accepted its hosted "opencode*" providers here; those are gone.
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @pairbuildr/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains pairbuildr-owned. The native runtime only adapts
      // the @pairbuildr/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
