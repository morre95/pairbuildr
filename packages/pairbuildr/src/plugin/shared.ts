...
export async function checkPluginCompatibility(target: string, pairbuildrVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(pairbuildrVersion) || semver.major(pairbuildrVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.pairbuildr
  if (typeof range !== "string") return
  if (!semver.satisfies(pairbuildrVersion, range)) {
    throw new Error(`Plugin requires pairbuildr ${range} but running ${pairbuildrVersion}`)
  }
}
