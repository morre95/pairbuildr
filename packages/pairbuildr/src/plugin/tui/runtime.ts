...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@pairbuildr/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@pairbuildr/core/effect/app-node-builder"
import { errorData, errorMessage } from "@pairbuildr/tui/util/error"
...
import { hasTheme, upsertTheme } from "@pairbuildr/tui/context/theme"
import { Global } from "@pairbuildr/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@pairbuildr/core/util/flock"
import { Flag } from "@pairbuildr/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@pairbuildr/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@pairbuildr/core/v1/config/plugin"
import { createCommandShim } from "@pairbuildr/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@pairbuildr/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".pairbuildr"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".pairbuildr", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".pairbuildr", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.PAIRBUILDR_PURE ? [] : pluginOrigins
    if (Flag.PAIRBUILDR_PURE && pluginOrigins.length) {
    }
