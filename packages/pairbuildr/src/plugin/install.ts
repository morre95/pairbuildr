...
export type PatchDeps = {
  ...
  files: (dir: string, name: "pairbuildr" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".pairbuildr")
}

function patchName(kind: Kind): "pairbuildr" | "tui" {
  if (kind === "server") return "pairbuildr"
  return "tui"
}
