...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from pairbuildr.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const pairbuildr = await pairbuildrFiles(input)
  for (const file of pairbuildr) {
...
async function pairbuildrFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "pairbuildr"),
    ...(await Filesystem.findUp(["pairbuildr.json", "pairbuildr.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "pairbuildr"))
  }
  if (Flag.PAIRBUILDR_CONFIG) files.push(Flag.PAIRBUILDR_CONFIG)
