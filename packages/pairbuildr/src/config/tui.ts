...
import { isRecord } from "@pairbuildr/tui/util/record"
...
import { TuiKeybind } from "@pairbuildr/tui/config/keybind"
...
import { TuiConfig } from "@pairbuildr/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@pairbuildr/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old pairbuildr.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.pairbuildr`
  // folders between cwd and home, and PAIRBUILDR_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.PAIRBUILDR_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit PAIRBUILDR_TUI_CONFIG override, if set.
  if (Flag.PAIRBUILDR_TUI_CONFIG) {
    const configFile = Flag.PAIRBUILDR_TUI_CONFIG
  ...
  // 4. `.pairbuildr` directories (and PAIRBUILDR_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".pairbuildr") || dir === Flag.PAIRBUILDR_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".pairbuildr") && dir !== Flag.PAIRBUILDR_CONFIG_DIR) continue
