...
import type * as SDK from "@pairbuildr/sdk/v2"
...
import { SessionShareTable } from "@pairbuildr/core/share/sql"
...
const disabled = process.env["PAIRBUILDR_DISABLE_SHARE"] === "true" || process.env["PAIRBUILDR_DISABLE_SHARE"] === "1"
...
export class Service extends Context.Service<Service, Interface>()("@pairbuildr/ShareNext") {}
  ...
  Effect.gen(function* () {
    ...
    const request = Effect.fn("ShareNext.request")(function* () {
      ...
      if (Option.isNone(active) || !active.value.active_org_id) {
        ...
        const baseUrl = config.share_url ?? config.enterprise?.url
        if (!baseUrl)
          return yield* Effect.die(
            new Error(
              "share_url is not configured. Sharing has no default host - set share_url in pairbuildr.json to a sharing service you control.",
            ),
          )
