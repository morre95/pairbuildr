...
import { PairBuildrHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for pairbuildr serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(PairBuildrHttpApi)
...
export const PublicApi = PairBuildrHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "PairBuildr",
    version: "1.0.0",
    description: "PairBuildr API",
