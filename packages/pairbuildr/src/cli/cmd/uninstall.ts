...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall and remove config, data, and cache",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall PairBuildr")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g pairbuildr",
      pnpm: "pnpm uninstall -g pairbuildr",
      bun: "bun remove -g pairbuildr",
      yarn: "yarn global remove pairbuildr",
      brew: "brew uninstall pairbuildr",
      choco: "choco uninstall pairbuildr",
      scoop: "scoop uninstall pairbuildr",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "pairbuildr", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using PairBuildr!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# pairbuildr") || content.includes(".pairbuildr/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".pairbuildr/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".pairbuildr/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".pairbuildr"))
    ) {
      continue
    }
