...
//
// Renders the two-line PairBuildr wordmark rules alongside session metadata and
// the resume command. These are scrollback snapshots, so they become immutable
// terminal history once committed.
//
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    const top = 1
    const body_left = RULE_TOP.length

    push(lines, 0, top, RULE_TOP, left)
    push(lines, 0, top + 1, RULE_BOTTOM, left)

    push(lines, body_left, top, "PairBuildr", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `pairbuildr --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
