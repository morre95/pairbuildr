import type { PermissionV1 } from "@pairbuildr/core/v1/permission"
import { FSUtil } from "@pairbuildr/core/fs-util"
// CLI entry point for `pairbuildr run` and `pairbuildr --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`pairbuildr --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`pairbuildr --mini --attach`): connects to a running
//      pairbuildr server and runs interactive mode against it.
//
...
import { createPairbuildrClient, type PairbuildrClient, type ToolPart } from "@pairbuildr/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<PairbuildrClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run one prompt and print the result",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running pairbuildr server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to PAIRBUILDR_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to PAIRBUILDR_SERVER_USERNAME or 'pairbuildr')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: PairbuildrClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: PairbuildrClient, sessionID: string) {
      ...
      async function current(sdk: PairbuildrClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: PairbuildrClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: PairbuildrClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: PairbuildrClient) {
        ...
        async function loop(client: PairbuildrClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createPairbuildrClient({
        baseUrl: "http://pairbuildr.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
