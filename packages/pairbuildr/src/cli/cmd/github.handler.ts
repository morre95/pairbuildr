...
const AGENT_USERNAME = "pairbuildr-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/pairbuildr.yml"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/pairbuildr-agent"
        const command =
        ...
        async function getInstallation() {
          const base = process.env["OIDC_BASE_URL"]
          if (!base)
            throw new Error(
              "OIDC_BASE_URL is not set. Installing the GitHub app requires a GitHub App API you control; there is no default.",
            )
...
jobs:
  pairbuildr:
    if: |
      ...
      contains(github.event.comment.body, ' /pairbuildr') ||
      startsWith(github.event.comment.body, '/pairbuildr')
    ...
    steps:
      ...
      - name: Run pairbuildr
        uses: <OWNER>/<REPO>/github@latest${envStr}
        with:
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    // Share links only appear when the user configured a share host; there is no default.
    const shareBaseUrl = process.env["PAIRBUILDR_SHARE_URL"] ?? ""
    ...
    try {
      ...
      console.log("pairbuildr session", session.id)
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value)
        throw new Error(
          "OIDC_BASE_URL is not set. The GitHub agent requires a GitHub App API you control; there is no default.",
        )
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/pairbuildr,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to pairbuildr...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("pairbuildr-github-action")
      } catch (error) {
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `pairbuildr/${type}-${hex}-${timestamp}`
      }
      return `pairbuildr/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      ...
      const shareUrl = shareId ? `[pairbuildr session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the pairbuildr infrastructure after your response",
