...
import { ConfigProvider } from "@pairbuildr/core/config/provider"
...
import { ConfigMigrateV1 } from "@pairbuildr/core/v1/config/migrate"
...
import { Policy } from "@pairbuildr/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads pairbuildr JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "pairbuildr.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "pairbuildr.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "pairbuildr.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "pairbuildr.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "pairbuildr.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "pairbuildr.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "pairbuildr.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .pairbuildr configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".pairbuildr"), { recursive: true })
            await fs.mkdir(path.join(directory, ".pairbuildr"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "pairbuildr.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "pairbuildr.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "pairbuildr.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "pairbuildr.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "pairbuildr.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".pairbuildr", "pairbuildr.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".pairbuildr", "pairbuildr.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".pairbuildr")),
              AbsolutePath.make(path.join(directory, ".pairbuildr")),
            ])
