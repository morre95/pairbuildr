...
import { Watcher } from "@pairbuildr/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    PAIRBUILDR_EXPERIMENTAL_FILEWATCHER: "true",
    PAIRBUILDR_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@pairbuildr.test`.cwd(tmp.path).quiet()
