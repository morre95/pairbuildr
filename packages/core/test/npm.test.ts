...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@pairbuildr/acme")).toBe("@pairbuildr/acme")
    expect(Npm.sanitize("@pairbuildr/acme@1.0.0")).toBe("@pairbuildr/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/pairbuildr/acme.git"
    const expected = win ? "acme@git+https_//github.com/pairbuildr/acme.git" : spec
