...
import { define } from "@pairbuildr/plugin/v2/effect"
...
import { LocationServiceMap } from "@pairbuildr/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "pairbuildr.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "pairbuildr.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
