...
const pairbuildrClient = process.env.PAIRBUILDR_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (pairbuildrClient === undefined) delete process.env.PAIRBUILDR_CLIENT
  else process.env.PAIRBUILDR_CLIENT = pairbuildrClient
})

describe("resource", () => {
  test("parses and decodes OTEL resource attributes", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "service.namespace=<OWNER>,team=platform%2Cobservability,label=hello%3Dworld,key%2Fname=value%20here"
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES = "service.namespace=<OWNER>,broken"

    expect(resource().attributes["service.namespace"]).toBeUndefined()
    expect(resource().attributes["pairbuildr.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.PAIRBUILDR_CLIENT = "cli"
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "pairbuildr.client=web,service.instance.id=override,service.namespace=<OWNER>"

    expect(resource().attributes).toMatchObject({
      "pairbuildr.client": "cli",
      "service.namespace": "<OWNER>",
    })
    expect(resource().attributes["service.instance.id"]).not.toBe("override")
    expect(resource().attributes["pairbuildr.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "pairbuildr-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
  }
  const file = path.join(dir, "pairbuildr.log")
