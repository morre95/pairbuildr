...
import { ToolFailure } from "@pairbuildr/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@pairbuildr/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.PAIRBUILDR_WEBSEARCH_PROVIDER === "exa" || process.env.PAIRBUILDR_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.PAIRBUILDR_WEBSEARCH_PROVIDER
        : undefined,
    enableExa:
      truthy("PAIRBUILDR_EXPERIMENTAL") || truthy("PAIRBUILDR_ENABLE_EXA") || truthy("PAIRBUILDR_EXPERIMENTAL_EXA"),
    enableParallel: truthy("PAIRBUILDR_ENABLE_PARALLEL") || truthy("PAIRBUILDR_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `PairBuildr/${InstallationVersion}`,
