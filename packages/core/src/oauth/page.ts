...
// etc.).
//
// The design tokens and wordmark below were inlined from the web component
// library when this repo was reduced to a CLI-only distribution. This file is
// now their only definition — update it directly when the brand changes.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `PairBuildr is now connected to ${escapeHtml(provider)}.` : "PairBuildr is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `PairBuildr couldn't finish connecting to ${escapeHtml(provider)}.`
        : "PairBuildr couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from PairBuildr.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · PairBuildr</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("PairBuildr couldn't finish connecting to "+PROVIDER+"."):"PairBuildr couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from PairBuildr."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("PairBuildr is now connected to "+PROVIDER+"."):"PairBuildr is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Curated subset of the OC-2 semantic tokens. Default is light; dark applies via
// prefers-color-scheme. The [data-theme] selectors let a host force a scheme
...
// PairBuildr wordmark, inlined as path geometry.
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 234 42" fill="none" aria-label="PairBuildr" role="img">
