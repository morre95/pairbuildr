...
This is the living design and status document for `@pairbuildr/codemode` and its existing V2 PairBuildr adapter.
...
### V2 PairBuildr adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/pairbuildr/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
