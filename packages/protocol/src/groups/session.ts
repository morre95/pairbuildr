...
import { PromptInput } from "@pairbuildr/schema/prompt-input"
import { Session } from "@pairbuildr/schema/session"
import { Project } from "@pairbuildr/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@pairbuildr/schema/schema"
...
import { SessionEvent } from "@pairbuildr/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this PairBuildr process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this PairBuildr process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
