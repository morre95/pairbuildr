import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@pairbuildr/plugin/tui"
...
import type { usePairbuildrKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof usePairbuildrKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getPairbuildrModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getPairbuildrModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
