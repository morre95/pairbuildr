...
export const PAIRBUILDR_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const PAIRBUILDR_MODE_KEY = "pairbuildr.mode"

export const PairbuildrKeymapProvider = KeymapProvider
export const usePairbuildrKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type PairbuildrModeStack = ReturnType<typeof createPairbuildrModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, PairbuildrModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createPairbuildrModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(PAIRBUILDR_MODE_KEY, PAIRBUILDR_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(PAIRBUILDR_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(PAIRBUILDR_MODE_KEY, stack.at(-1)?.mode ?? PAIRBUILDR_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? PAIRBUILDR_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(PAIRBUILDR_MODE_KEY, undefined)
...
export function usePairbuildrModeStack() {
  return getPairbuildrModeStack(usePairbuildrKeymap())
}

export function getPairbuildrModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Pairbuildr mode stack is not registered for this keymap")
  return value
}
...
export function registerPairbuildrKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createPairbuildrModeStack(keymap)
