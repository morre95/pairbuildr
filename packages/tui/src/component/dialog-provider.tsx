...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@pairbuildr/sdk/v2"
...
// Upstream ranked its own hosted providers first. With those gone the order is
// just the providers people most often bring their own key for.
...
const CUSTOM_PROVIDER_OPTION_VALUE = "__pairbuildr_custom_provider__"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in pairbuildr.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in pairbuildr.json to use it.`,
          })
          dialog.clear()
          return
        }
