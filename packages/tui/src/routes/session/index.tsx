...
import { PAIRBUILDR_BASE_MODE, useBindings, useCommandShortcut, usePairbuildrKeymap } from "../../keymap"
import { usePathFormatter } from "../../context/path-format"
import { LocationProvider } from "../../context/location"

addDefaultParsers(parsers.parsers)

const RETRY_NOTICE_FREE_TIER_LAST_SEEN_AT = "retry_notice_free_tier_last_seen_at"
const RETRY_NOTICE_FREE_TIER_DONT_SHOW = "retry_notice_free_tier_dont_show"
const RETRY_NOTICE_RATE_LIMIT_LAST_SEEN_AT = "retry_notice_rate_limit_last_seen_at"
const RETRY_NOTICE_RATE_LIMIT_DONT_SHOW = "retry_notice_rate_limit_dont_show"
const RETRY_NOTICE_WINDOW = 86_400_000 // 24 hrs

export const alwaysSeparate = new WeakSet<BoxRenderable>()

type RetryAction = Extract<SessionStatus, { type: "retry" }>["action"]

// Upstream gated this on its own hosted providers so the dialog could advertise a
// subscription. The notice itself is useful for any provider that reports a limit,
// so it is now provider-neutral and keyed only by the reason.
function retryNoticeKeys(action: RetryAction) {
  if (!action) return
  if (action.reason === "free_tier_limit") {
    return {
      lastSeenAt: RETRY_NOTICE_FREE_TIER_LAST_SEEN_AT,
      dontShow: RETRY_NOTICE_FREE_TIER_DONT_SHOW,
    }
  }
  if (action.reason === "account_rate_limit") {
    return {
      lastSeenAt: RETRY_NOTICE_RATE_LIMIT_LAST_SEEN_AT,
      dontShow: RETRY_NOTICE_RATE_LIMIT_DONT_SHOW,
    }
  }
}
...
export function Session() {
  ...
  const keymap = usePairbuildrKeymap()
  ...
  event.on("session.status", (evt) => {
    ...
    const keys = retryNoticeKeys(evt.properties.status.action)
    if (!keys) return

    const seen = kv.get(keys.lastSeenAt)
    if (typeof seen === "number" && Date.now() - seen < RETRY_NOTICE_WINDOW) return
