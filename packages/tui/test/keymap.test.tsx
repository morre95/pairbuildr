...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerPairbuildrKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when pairbuildr mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getPairbuildrModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getPairbuildrModeStack(keymap).push("autocomplete")
