...
> current private `@pairbuildr/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@pairbuildr/ai`
...
- Secondary audience: PairBuildr and other durable agent runtimes
...
import { LLM } from "@pairbuildr/ai"
import { OpenAI } from "@pairbuildr/ai/providers/openai"
...
```ts
import { Message } from "@pairbuildr/ai"
...
import { LLM, Tool } from "@pairbuildr/ai"
...
PairBuildr and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@pairbuildr/ai/promise"
import { OpenAI } from "@pairbuildr/ai/providers/openai"
...
```ts
import { LLMSchema } from "@pairbuildr/ai/schema"
...
```ts
import { Provider, Protocol } from "@pairbuildr/ai/provider"
...
@pairbuildr/ai/promise
  Promise/AsyncIterable LLM facade

@pairbuildr/ai/schema
  serializable domain schemas

@pairbuildr/ai/provider
  experimental Provider and Protocol authoring APIs

@pairbuildr/ai/providers/openai
@pairbuildr/ai/providers/anthropic
@pairbuildr/ai/providers/google
...
```
...
PairBuildr should migrate to `generateTurn` / `streamTurn`, preserving its durable
